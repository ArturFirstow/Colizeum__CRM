...
// ─────────────────────────────────────────────────────────────────────────────
// Рекламные форматы COLIZEUM — то, что мы продаём клиенту.
//
// Список дал пользователь; ничего не добавлено от себя. Используется галочками
// в запросах юристу и на размещение макетов (см. src/lib/request-templates.ts).
//
// needsOrd — формат интернетовский, значит нужна маркировка и ЕРИД. Правило
// домена: маркировка только для интернет-форматов, экраны в клубе под неё
// не подпадают.
// ─────────────────────────────────────────────────────────────────────────────
export type AdFormat = {
  label: string;
  needsOrd: boolean;
  /** Особое условие формата — уходит в текст запроса отдельной строкой. */
  note?: string;
};

export const AD_FORMATS: AdFormat[] = [
  { label: "Баннер в личном кабинете (некликабельный, статичный)", needsOrd: false },
  { label: "Кликабельный баннер (или видео) на рабочем столе", needsOrd: false },
  { label: "Статичный / анимированный баннер на экране ПК", needsOrd: false },
  { label: 'Баннер на TV 65" (статика)', needsOrd: false },
  { label: "Баннер в приложении COLIZEUM, кликабельный", needsOrd: true },
  { label: "Реклама в группе Colizeum в Telegram (@colizeumarena)", needsOrd: true },
  { label: "Реклама в группе Colizeum во ВКонтакте (vk.com/colizeumarena)", needsOrd: true },
  { label: "Брендинг браузера (без ротации)", needsOrd: false },
  { label: "Брендирование рабочего стола", needsOrd: false, note: "новый формат" },
  {
    label: "Интеграция в прайс: баннер на ресепшен с прайсом",
    needsOrd: false,
    note: "срок запуска — 1 месяц после согласования формата",
  },
