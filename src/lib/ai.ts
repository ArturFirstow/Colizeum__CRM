...
/**
 * Понятное сообщение, если ключ не задан. Называем провайдера, который РЕАЛЬНО
 * выбран в .env, а не абстрактный пример: иначе выходит совет подключить то,
 * от чего уже отказались.
 */
export const AI_NO_KEY_MESSAGE =
  `ИИ не подключён: у провайдера «${PROVIDER}» не задан ключ. ` +
  'Добавьте в .env строку AI_API_KEY="…" и перезапустите сервис. ' +
  "Проверить подключение можно командой: npm run ai:check";
...
export function aiErrorMessage(e: unknown): string {
  ...
  if (status === 403 || /request not allowed|forbidden/i.test(raw)) {
    return (
      `Провайдер «${PROVIDER}» отклонил запрос (403) — обычно это значит, что обращения из вашей страны не принимаются. ` +
      "Ключ при этом рабочий. Из России стабильно доступны deepseek и yandex — поставьте " +
      'AI_PROVIDER="deepseek" в .env, либо запускайте сервис на зарубежном сервере.'
    );
  }
  if (status === 401) {
    return `Провайдер «${PROVIDER}» не принял ключ (401): проверьте, что AI_API_KEY скопирован целиком и не отозван в кабинете провайдера.`;
  }
