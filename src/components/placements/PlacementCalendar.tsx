...
export function PlacementCalendar({
  ...
  const model = useMemo(() => {
    if (placements.length === 0) {
      const now = new Date();
      const base = ym(now);
      return { baseMonth: base, monthCount: 6 };
    }
    let min = Infinity;
    let max = -Infinity;
    for (const p of placements) {
      min = Math.min(min, ym(new Date(p.startDate)));
      max = Math.max(max, ym(new Date(p.endDate)));
    }
    return { baseMonth: min, monthCount: max - min + 1 };
  }, [placements]);
  ...
  // Строки-слоты в порядке справочника, плюс любые нестандартные в конце.
  const slots = useMemo(() => {
    const present = [...new Set(placements.map((p) => p.slot))];
    const ordered = (PLACEMENT_SLOTS as readonly string[]).filter((s) => present.includes(s));
    ...
    <section className="card p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-ink-500">Тайминг по слотам и неделям, как в рабочей таблице</p>
        </div>
      ...
      {slots.length === 0 ? (
        <div className="rounded-xl border border-dashed border-ink-700 py-10 text-center text-sm text-ink-400">
          Броней нет. Добавьте первую — «+ Бронь».
        </div>
      ) : (
        <div className="overflow-x-auto pb-2">
          <div style={{ minWidth: LABEL_W + totalWeeks * WEEK_W }}>
            ...
            {/* Строки слотов */}
            {slots.map((slot, si) => {
              const rowPlacements = placements.filter((p) => p.slot === slot);
              return (
                <div
                  key={slot}
                ...
                >
                  ...
                  {rowPlacements.map((p) => {
                    ...
                    const e = Math.min(totalWeeks - 1, weekIndex(new Date(p.endDate)));
                    const brand = p.brandLabel ?? p.advertiser?.nameRu ?? "—";
                    return (
                      <button
                        key={p.id}
                        onClick={() => setEdit(p)}
                        style={{ gridColumn: `${2 + s} / ${2 + Math.max(s, e) + 1}`, gridRow: 1 }}
                        title={`${brand} · ${p.status}`}
                        className={`z-[1] mx-0.5 my-1 truncate rounded-md px-1.5 py-1 text-[11px] font-semibold ring-1 ring-black/20 transition hover:brightness-110 ${STATUS_BAR[p.status] ?? "bg-ink-500/70 text-ink-50"}`}
                      >
                        {brand}
                      </button>
                    );
      ...
      />
      {edit && (
        <EditPlacementModal placement={edit} onClose={() => setEdit(null)} onChanged={() => router.refresh()} />
      )}
    </section>
  );
}
...
function EditPlacementModal({
  ...
  async function save() {
    setSaving(true);
    try {
      await apiFetch(`/api/placements/${placement.id}`, { method: "PATCH", body: JSON.stringify({ status }) });
      onChanged();
      onClose();
    } finally {
      setSaving(false);
  ...
  return (
    <Modal open onClose={onClose} title={`${brand} · ${placement.slot}`} size="sm">
      <div className="space-y-4">
        <div className="text-sm text-ink-300">
          {formatDate(placement.startDate)} — {formatDate(placement.endDate)}
          {placement.responsible ? ` · ${placement.responsible}` : ""}
        </div>
        <div>
          <label className="label">Статус</label>
