...
for (let bar = 0; bar < BARS; bar++) {
  ...
  const full = bar >= 4;            // «дроп» — с 5-го такта
  const beatIn = bar >= 2;          // бочка появляется с 3-го
  const peak = bar >= 8;            // самая плотная часть

  pad(t, chord.tones, BAR + 0.1, bar < 2 ? 0.14 : 0.085);
  ...
  if (bar >= 1) {
    ...
    for (let s = 0; s * step < BAR; s++) {
      ...
      hat(t + s * step, open, open ? 0.16 : bar < 4 ? 0.16 : 0.22);
    }
  }
  ...
  if (bar >= 4) {
    ...
    for (let s = 0; s < 8; s++) {
      ...
      pluck(t + s * (BEAT / 2), f, 0.4, peak ? 0.2 : 0.15, s % 2 ? 0.25 : -0.25);
      if (peak) pluck(t + s * (BEAT / 2), f * 2, 0.26, 0.075, s % 2 ? -0.3 : 0.3);
    }
  }

  if (bar === 3 || bar === 7 || bar === 11) riser(t + BAR - 1.4, 1.4, bar === 3 ? 0.34 : 0.24);
  if (bar === 4 || bar === 8 || bar === 12) impact(t, bar === 4 ? 0.75 : 0.5);
}

// финальный удар и хвост
impact(BARS * BAR - BAR, 0.6);
...
console.log("music.wav", (buf.length / 1e6).toFixed(2), "MB,", DUR.toFixed(2), "s");
