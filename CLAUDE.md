...
- **Кабинет руководителя** (`/leadership`): обзор отдела (по-сотрудная сводка + воронка), бюджет отдела (`/leadership/budget`: доходы/расходы + CSV-выгрузка, формулы из Google-таблицы в `src/lib/dept-budget.ts`).
