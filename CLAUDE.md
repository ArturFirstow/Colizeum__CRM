...
4. **ИИ-чат ассистент** (1B): окно чата с контекстом кабинета. ← следующий
...
**НЕ сделано (следующие итерации):** ИИ-чат ассистент, RBAC + аудит-лог, S3/прод-деплой, realtime на несколько инстансов (сейчас in-memory, для 1 сервера). **Ждём от пользователя:** `ANTHROPIC_API_KEY` в `.env` для ИИ-кнопок; база знаний коллеги.
