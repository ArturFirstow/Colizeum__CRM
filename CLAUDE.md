...
**Проверка ИИ:** `npm run ai:check` — делает один тестовый запрос к провайдеру и печатает вердикт (что настроено, куда ушёл запрос, что ответил провайдер). Быстрее, чем гадать через интерфейс.
