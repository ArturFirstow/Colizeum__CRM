...
**Дорожная карта (согласовано с пользователем, реализуем по порядку):**
1. ✅ Задачи от руководителя.
2. **ИИ-кнопки в карточках** (1A): «Саммари клиента за месяц», «Драфт ДС по шаблону». Нужен ИИ-ключ. ← следующий
3. ✅ Внутренний мессенджер (каналы, вложения, закрепы, привязка к клиенту/сделке; личка/realtime — позже).
4. **ИИ-чат ассистент** (1B): окно чата с контекстом кабинета.

**ИИ-провайдер (не выбран):** обвязку делать провайдер-независимой (OpenAI-совместимый API — можно Gemini/Groq бесплатно, DeepSeek дёшево, GigaChat/YandexGPT если «данные в РФ»). Открытые вопросы к пользователю: (1) важна ли data-residency РФ; (2) какой провайдер первым.

**НЕ сделано (следующие итерации):** ИИ-функции (см. дорожную карту), RBAC + аудит-лог, фоновые задачи, S3/прод-деплой. **Ждём от пользователя:** база знаний коллеги и файлы «материалов для клиента» (структура в БЗ готова, категория «Материалы для клиента»).
